...
# Update these interface names to match your machine ('ip addr')
...
def run_worker(rank, world_size, master_addr, master_port, device):
    ...
    # Enable direct GPU-to-GPU transfer if available
    if torch.cuda.is_available() and "cuda" in device:
        # Maps "master:cuda:0" -> "worker:cuda:0" (adjust keys if master uses different index)
