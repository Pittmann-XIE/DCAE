...
# # Copy all the building block classes from Code 1
...
#     """Standalone compression model for edge deployment"""
...
#         # Hyperprior decoders (needed for parameter estimation)
...
#         # Update entropy bottleneck first
#         self.entropy_bottleneck.update(force=force)
#         # Update gaussian conditional
...
#         """Load state dict with entropy model buffer updates"""
...
#     """Standalone decompression model for server deployment"""
...
# class ParameterSync:
#     """Utility class for synchronizing parameters between CompressModel and DecompressModel"""
    
#     @staticmethod
#     def copy_shared_components(source_model, target_model):
#         """Copy shared components from source model to target model"""
#         # Copy dictionary parameters
#         target_model.dt.data = source_model.dt.data.clone()
        
#         # Copy cross-attention modules
#         for i in range(len(source_model.dt_cross_attention)):
#             target_model.dt_cross_attention[i].load_state_dict(
#                 source_model.dt_cross_attention[i].state_dict()
#             )
        
#         # Copy parameter estimation networks
#         for i in range(len(source_model.cc_mean_transforms)):
#             target_model.cc_mean_transforms[i].load_state_dict(
#                 source_model.cc_mean_transforms[i].state_dict()
#             )
#             target_model.cc_scale_transforms[i].load_state_dict(
#                 source_model.cc_scale_transforms[i].state_dict()
#             )
#             target_model.lrp_transforms[i].load_state_dict(
#                 source_model.lrp_transforms[i].state_dict()
#             )
        
#         # Copy hyperprior decoders
#         target_model.h_z_s1.load_state_dict(source_model.h_z_s1.state_dict())
#         target_model.h_z_s2.load_state_dict(source_model.h_z_s2.state_dict())
        
#         # Copy entropy models
#         target_model.entropy_bottleneck.load_state_dict(source_model.entropy_bottleneck.state_dict())
#         target_model.gaussian_conditional.load_state_dict(source_model.gaussian_conditional.state_dict())
...
#         # Get target device (GPU for decompress model)
#         target_device = next(decompress_model.parameters()).device
#         print(f'syn compress to decompress: target device: {target_device}')
        
#         # Copy dictionary parameters and move to target device
#         decompress_model.dt.data = compress_model.dt.data.clone().to(target_device)
        
#         # Copy other shared components and ensure they're on the right device
#         for i in range(len(compress_model.dt_cross_attention)):
#             decompress_model.dt_cross_attention[i].load_state_dict(
#                 compress_model.dt_cross_attention[i].state_dict()
#             )
#             decompress_model.dt_cross_attention[i] = decompress_model.dt_cross_attention[i].to(target_device)
        
#         # Copy parameter estimation networks
#         for i in range(len(compress_model.cc_mean_transforms)):
#             decompress_model.cc_mean_transforms[i].load_state_dict(
#                 compress_model.cc_mean_transforms[i].state_dict()
#             )
#             decompress_model.cc_mean_transforms[i] = decompress_model.cc_mean_transforms[i].to(target_device)
            
#             decompress_model.cc_scale_transforms[i].load_state_dict(
#                 compress_model.cc_scale_transforms[i].state_dict()
#             )
#             decompress_model.cc_scale_transforms[i] = decompress_model.cc_scale_transforms[i].to(target_device)
            
#             decompress_model.lrp_transforms[i].load_state_dict(
#                 compress_model.lrp_transforms[i].state_dict()
#             )
#             decompress_model.lrp_transforms[i] = decompress_model.lrp_transforms[i].to(target_device)
        
#         # Copy hyperprior decoders
#         decompress_model.h_z_s1.load_state_dict(compress_model.h_z_s1.state_dict())
#         decompress_model.h_z_s1 = decompress_model.h_z_s1.to(target_device)
        
#         decompress_model.h_z_s2.load_state_dict(compress_model.h_z_s2.state_dict())
#         decompress_model.h_z_s2 = decompress_model.h_z_s2.to(target_device)
        
#         # Copy entropy models
#         decompress_model.entropy_bottleneck.load_state_dict(compress_model.entropy_bottleneck.state_dict())
#         decompress_model.entropy_bottleneck = decompress_model.entropy_bottleneck.to(target_device)
        
#         decompress_model.gaussian_conditional.load_state_dict(compress_model.gaussian_conditional.state_dict())
#         decompress_model.gaussian_conditional = decompress_model.gaussian_conditional.to(target_device)

#         # Copy entropy bottleneck buffers
#         decompress_model.entropy_bottleneck._quantized_cdf = compress_model.entropy_bottleneck._quantized_cdf.clone().to(target_device)
#         decompress_model.entropy_bottleneck._offset = compress_model.entropy_bottleneck._offset.clone().to(target_device)
#         decompress_model.entropy_bottleneck._cdf_length = compress_model.entropy_bottleneck._cdf_length.clone().to(target_device)

#         # Copy gaussian conditional buffers
#         decompress_model.gaussian_conditional._quantized_cdf = compress_model.gaussian_conditional._quantized_cdf.clone().to(target_device)
#         decompress_model.gaussian_conditional._offset = compress_model.gaussian_conditional._offset.clone().to(target_device)
#         decompress_model.gaussian_conditional._cdf_length = compress_model.gaussian_conditional._cdf_length.clone().to(target_device)


#     @staticmethod
#     def sync_decompress_to_compress(decompress_model, compress_model):
#         """Sync parameters from decompression model to compression model"""
#         ParameterSync.copy_shared_components(decompress_model, compress_model)

#     @staticmethod
#     def save_shared_parameters(model, path):
#         """Save only shared parameters that need to be transferred between models"""
#         shared_params = {
#             'dt': model.dt.cpu(),
#             'dt_cross_attention': [module.state_dict() for module in model.dt_cross_attention],
#             'cc_mean_transforms': [module.state_dict() for module in model.cc_mean_transforms],
#             'cc_scale_transforms': [module.state_dict() for module in model.cc_scale_transforms],
#             'lrp_transforms': [module.state_dict() for module in model.lrp_transforms],
#             'h_z_s1': model.h_z_s1.state_dict(),
#             'h_z_s2': model.h_z_s2.state_dict(),
#             'entropy_bottleneck_state': model.entropy_bottleneck.state_dict(),
#             'gaussian_conditional_state': model.gaussian_conditional.state_dict(),
#             # buffers
#             'entropy_bottleneck__quantized_cdf': getattr(model.entropy_bottleneck, '_quantized_cdf', None),
#             'entropy_bottleneck__offset': getattr(model.entropy_bottleneck, '_offset', None),
#             'entropy_bottleneck__cdf_length': getattr(model.entropy_bottleneck, '_cdf_length', None),
#             'gaussian_conditional__quantized_cdf': getattr(model.gaussian_conditional, '_quantized_cdf', None),
#             'gaussian_conditional__offset': getattr(model.gaussian_conditional, '_offset', None),
#             'gaussian_conditional__cdf_length': getattr(model.gaussian_conditional, '_cdf_length', None),
#         }
#         torch.save(shared_params, path)


#     @staticmethod
#     def load_shared_parameters(model, path):
#         """Load shared parameters into a model"""
#         shared_params = torch.load(path, map_location='cpu')
        
#         model.dt.data = shared_params['dt']
        
#         for i, state_dict in enumerate(shared_params['dt_cross_attention']):
#             model.dt_cross_attention[i].load_state_dict(state_dict)
        
#         for i, state_dict in enumerate(shared_params['cc_mean_transforms']):
#             model.cc_mean_transforms[i].load_state_dict(state_dict)
            
#         for i, state_dict in enumerate(shared_params['cc_scale_transforms']):
#             model.cc_scale_transforms[i].load_state_dict(state_dict)
            
#         for i, state_dict in enumerate(shared_params['lrp_transforms']):
#             model.lrp_transforms[i].load_state_dict(state_dict)
        
#         model.h_z_s1.load_state_dict(shared_params['h_z_s1'])
#         model.h_z_s2.load_state_dict(shared_params['h_z_s2'])
#         model.entropy_bottleneck.load_state_dict(shared_params['entropy_bottleneck'])
#         model.gaussian_conditional.load_state_dict(shared_params['gaussian_conditional'])



## in CompressModel.forward, use z_hat, z_likelihoods = self.entropy_bottleneck(z).
