...
def parse_args(argv):
    ...
    parser.add_argument("--type", type=str, default='mss-ssim', help="loss type", choices=['mse', "ms-ssim", "l1"])
    parser.add_argument("--save_path", type=str, help="save_path", default='./checkpoints/dace_7')
    parser.add_argument(
...
def main(argv):
    ...
    if args.checkpoint:  
            ...
            # 1. Load Weights (Keep this)
            net.load_state_dict(checkpoint["state_dict"])

            if args.continue_train:
                last_epoch = checkpoint["epoch"] + 1
                
                print(f"[MIGRATION FIX] Skipping Optimizer State due to architecture change.")
                print(f"Resuming at Epoch {last_epoch} with fresh Optimizer.")

                # 2. DO NOT load optimizer/aux_optimizer state.
                # optimizer.load_state_dict(checkpoint["optimizer"])      # DISABLED
                # aux_optimizer.load_state_dict(checkpoint["aux_optimizer"])  # DISABLED
                
                # 3. Manually sync the Scheduler to the correct epoch
                # We re-create the scheduler with 'last_epoch' set correctly.
                # This forces it to calculate the correct LR (e.g. 1e-5) immediately.
